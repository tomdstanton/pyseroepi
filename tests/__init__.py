...
def test():
    ...
    # from seroepi._formulation import PostHocFormulationDesigner as Designer
    #
    # est = Estimator()
    # agg = df.epi.aggregate_prevalence(stratify_by=['spatial', 'K_locus'], cluster_col=t_clusters.name)
    # res = est.calculate(agg)
    # res.plot(PlotType.COMPOSITION_BAR)
    # vax = Designer(valency=20)
    # _formulation = vax.evaluate(res, 'spatial')
    # _formulation.plot('rank_stability_bump')
    #
    #
    # from seroepi.estimators import BayesianPrevalenceEstimator as Estimator
    # from seroepi._formulation import CVFormulationDesigner
    # est = Estimator()
    # agg = df.epi.aggregate_prevalence(stratify_by=['spatial', 'K_locus'])
    # res = est.calculate(agg)
    # est.diagnostics()
    # vax = Designer()
    # _formulation = vax.evaluate(est, agg, 'spatial')
